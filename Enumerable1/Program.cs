...
var EmployeesWithVisa = employeeList.Filter(
    (card, employee) => employee.HireDate > new DateTime(2023, 1, 1) && 
                        card.Issuer == CardIssuer.Visa
);
Console.WriteLine("\n=== Employees Hired After 2023 with Visa ===");
foreach (var employee in EmployeesWithVisa)
{
    Console.WriteLine($"{employee.FirstName} {employee.LastName} - {employee.Role}");
    var visaCards = employee.CreditCards.Where(c => c.Issuer == CardIssuer.Visa);
    foreach (var card in visaCards)
    {
        Console.WriteLine($"  VISA: {card.Number} (Exp: {card.ExpirationMonth}/{card.ExpirationYear})");
    }
}
